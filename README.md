...
Ниже короткая инструкция, как поднять пуллинг-сервер из примера на своем сервере и подключить к нему софт.
...
```bash
python3 anygram_polling.py
```

или через uv:

```bash
uv run anygram_polling.py
```


Далее пуллинг-сервер станет доступен для софта по пути 
```
http://{IP}:4546/anygram_poll
```
или 
```
https://{IP}:4546/anygram_poll (если задали в настройках)
```

Если порты открыты то можно вставлять эту ссылку в софт в Polling URL.

Если вы проксируете трафик с домена то ссылка будет:
```
https://{ВАШ ДОМЕН}/anygram_poll
```

После успешного подключения Ваш Веб-интерфейс и API софта станут доступны по
...
Далее можно перейти по этой ссылке, ввести ваш апи ключ и пользоваться софтом удаленно с сервера.
Либо пользоваться всем функционалом API софта на сервре отправляя запросы по пути http://127.0.0.1:4546/panel_api/ с других приложений на вашем сервере.
Если вам нужно проксировать только API запросы и вы не хотите проксировать веб-интерфейс, установите в коде данную переменную на True
```
DEFAULT_ALLOW_API_ONLY=True
```
Тогда будет проксироваться только API.
